...
from datasets import open_weiboscope 
...
def run_tests():
    ...
    if(model_name == 'char_cnn'):
        (opentr, opente),(tr_size, te_size) = char_input(batch_size=batch_size)
        model,sgd = char_cnn()
        add_dim_to_input = True
        halve_lr = True

    elif(model_name == 'glove_cnn'):
        (opentr, opente),(tr_size, te_size) = word_embedding_input('glove',batch_size=batch_size,num_words=num_words)
        model,sgd = word_cnn('glove',num_words)
        add_dim_to_input = True
        halve_lr = True

    elif(model_name == 'word2vec_cnn'):
        (opentr, opente),(tr_size, te_size) = word_embedding_input('word2vec',batch_size=batch_size,num_words=num_words)
        model,sgd = word_cnn('word2vec',num_words)
        add_dim_to_input = True
        halve_lr = True

    elif(model_name == 'char_lstm'):
        (opentr, opente),(tr_size, te_size) = char_input(batch_size=batch_size,chars_reversed=False)
        model = char_lstm()
    ...
    test_confusions = []
    costs = []    
    save_path = 'open/'+ model_name + '/model.json'
    model_weights = 'open/'+ model_name + '/model_weights.hd5'
...
def word_embedding_input(embedding_type,batch_size=30,num_words=99):
    ...
    '''(opentr, opente),(train_size, test_size) = batch_data.split_and_batch(
        data_loader=None,doclength=None,batch_size=batch_size,h5_path='open_split.hd5',
    ...
    open_train_batch = BatchIterator(opentr,
    ...
    open_test_batch = BatchIterator(opente,
            normalizer_fun=lambda x: data_utils.normalize(x, 
                min_length=100,
                max_length=1014, 
                truncate_left=False,
    ...
    return (open_train_batch, open_test_batch),(train_size, test_size)
