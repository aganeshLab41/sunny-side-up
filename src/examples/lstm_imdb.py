...
THEANO_FLAGS=mode=FAST_RUN,device=gpu1,floatX=float32 python lstm_imdb.py
'''
...
def model_defn():
    ...
    model.add(LSTM(1,init='orthogonal',forget_bias_init='one',activation='tanh',
...
if __name__=="__main__":
    ...
    '''(amtr,amte),(amntr,amnte) = datasets, sizes = batch_data.split_data(None,
        h5_path='imdb_split.hd5', overwrite_previous=False,shuffle=True)
    ...
    (imdbtr, imdbte),(train_size, test_size) = batch_data.split_and_batch(
        data_loader=None,doclength=None,batch_size=128,h5_path='imdb_split.hd5')
    
    print("Train size", train_size)
    print("Test size", test_size)

    #tweet, sentiment = train_batcher().next()
