...
'''
THEANO_FLAGS=mode=FAST_RUN,device=gpu0,floatX=float32 python amazon_run.py
'''
...
if __name__=="__main__":
    ...
    (amztr, amzte),(tr,te) = batch_data.split_data(DataIterator(amzData.load_data),
        h5_path='amazon_split.hd5',
        overwrite_previous=False,
        shuffle=True)    

    amz_train_batch = BatchIterator(amztr,
            normalizer_fun=lambda x: data_utils.normalize(x, 
            min_length=100,
            max_length=1014, 
            truncate_left=True,
    ...
    text,label = amz_train_batch.next()
